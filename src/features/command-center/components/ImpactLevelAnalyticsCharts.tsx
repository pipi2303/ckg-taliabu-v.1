    ...
    <div className="space-y-6 pt-2">
      ...
      <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900/95 to-slate-950 border border-slate-800 shadow-xl space-y-4">
            ...
            <div className="flex items-center bg-slate-800/90 rounded-xl p-1 border border-slate-700 text-xs">
              <button
                ...
                className={`px-3 py-1 rounded-lg font-medium transition cursor-pointer ${
                  geoFilter === 'ALL' ? 'bg-teal-600 text-white shadow-xs' : 'text-slate-400 hover:text-white'
                }`}
              >
                Semua Wilayah (8)
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-medium transition cursor-pointer ${
                  geoFilter === 'MAINLAND' ? 'bg-teal-600 text-white shadow-xs' : 'text-slate-400 hover:text-white'
                }`}
              >
                Daratan (4)
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-medium transition cursor-pointer ${
                  geoFilter === 'REMOTE' ? 'bg-teal-600 text-white shadow-xs' : 'text-slate-400 hover:text-white'
                }`}
              >
                Pesisir/Pulau (4)
              </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
          ...
          <div className="p-3 rounded-xl bg-teal-950/40 border border-teal-800/50 flex items-center justify-between">
            ...
            <div className="p-2 rounded-lg bg-teal-500/10 text-teal-300">
              <Activity className="w-4 h-4" />
            </div>
          </div>

          {/* Level 2 Summary Pill */}
          <div className="p-3 rounded-xl bg-sky-950/40 border border-sky-800/50 flex items-center justify-between">
            <div className="space-y-0.5">
          ...
          <div className="p-3 rounded-xl bg-amber-950/40 border border-amber-800/50 flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <div className="text-sm font-bold text-amber-200 flex items-center gap-1.5 mt-1">
              ...
              <p className="text-[10px] text-slate-400">290 pasien aktif dalam kohort pemantauan</p>
            </div>
            ...
            <div className="flex items-center gap-2 self-start sm:self-center">
              <div className="flex items-center bg-slate-800 rounded-xl p-1 border border-slate-700 text-xs">
                <button
                  ...
                  className={`px-3 py-1 rounded-lg font-medium transition cursor-pointer ${
                    barSubMode === 'VOLUME' ? 'bg-teal-600 text-white shadow-xs' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Beban & Gap Warga
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-lg font-medium transition cursor-pointer ${
                    barSubMode === 'RISK_STACK' ? 'bg-teal-600 text-white shadow-xs' : 'text-slate-400 hover:text-white'
                  }`}
                >
              ...
              {barSubMode === 'VOLUME' ? (
                <BarChart data={filteredFacilities} margin={{ top: 10, right: 20, left: 0, bottom: 25 }}>
                  ...
                  <XAxis
                    ...
                    textAnchor="end"
                  />
                  <YAxis stroke="#94a3b8" fontSize={11} tickLine={false} unit=" jiwa" />
                  <RechartsTooltip
                    contentStyle={{
                  ...
                  />
                  <Bar dataKey="targetPopulation" name="Sasaran Registrasi (Populasi)" fill="#475569" radius={[4, 4, 0, 0]} />
                  ...
                  <Bar dataKey="gapL2" name="Kesenjangan Belum Ditangani (Gap)" fill="#f43f5e" radius={[4, 4, 0, 0]} />
                </BarChart>
              ) : (
                <BarChart data={filteredFacilities} margin={{ top: 10, right: 20, left: 0, bottom: 25 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#334155" opacity={0.5} />
                  ...
                  <Bar dataKey="criticalRisk" stackId="a" name="Risiko Kritis (Merah Gelap)" fill="#e11d48" radius={[4, 4, 0, 0]} />
                </BarChart>
              )}
            </ResponsiveContainer>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800 text-xs text-slate-300 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-teal-400 hover:text-teal-300 font-semibold text-xs transition flex items-center gap-1 shrink-0"
            >
      ...
      {(activeTab === 'ALL' || activeTab === 'AREA') && (
        <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
            <div>
              <h4 className="text-sm font-bold text-white flex items-center gap-2">
                <AreaChartIcon className="w-4 h-4 text-sky-400" />
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center bg-slate-800 rounded-xl p-1 border border-slate-700 text-xs">
                <button
                  ...
                  className={`px-3 py-1 rounded-lg font-medium transition cursor-pointer ${
                    areaSubMode === 'CUMULATIVE' ? 'bg-sky-600 text-white shadow-xs' : 'text-slate-400 hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-lg font-medium transition cursor-pointer ${
                    areaSubMode === 'CASCADE' ? 'bg-sky-600 text-white shadow-xs' : 'text-slate-400 hover:text-white'
                  }`}
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase font-semibold">Akselerasi Skrining (L1)</span>
              <p className="font-bold text-teal-400 text-sm">+768 warga diskrining sejak Jan 2026</p>
              <p className="text-[11px] text-slate-400">Rata-rata 109 warga/bulan terlayani</p>
            </div>
            <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase font-semibold">Tindak Lanjut Faskes (L2)</span>
              <p className="font-bold text-sky-400 text-sm">268 pasien berisiko telah tertata laksana</p>
              <p className="text-[11px] text-slate-400">61.3% tingkat kontinuitas kabupaten</p>
            </div>
            <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase font-semibold">Jalur Outcome Terkendali (L3)</span>
              <p className="font-bold text-amber-400 text-sm">290 pasien dalam kohort 90-hari</p>
              <p className="text-[11px] text-slate-400">Menunggu audit validasi CR-OC (OI-08)</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="h-80 w-full pt-2">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={MONTHLY_TREND_DATA} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#334155" opacity={0.5} />
                <XAxis dataKey="month" stroke="#94a3b8" fontSize={11} tickLine={false} />
                <YAxis yAxisId="left" stroke="#94a3b8" fontSize={11} tickLine={false} unit=" jiwa" />
                <YAxis yAxisId="right" orientation="right" stroke="#10b981" fontSize={11} tickLine={false} domain={[0, 100]} unit="%" />
                <RechartsTooltip
                  contentStyle={{
                    backgroundColor: '#0f172a',
                    borderColor: '#334155',
                ...
                />
                <Legend verticalAlign="top" height={36} wrapperStyle={{ fontSize: '11px', color: '#cbd5e1' }} />
                <ReferenceLine yAxisId="right" y={50} stroke="#f59e0b" strokeDasharray="4 4" label={{ value: 'Target SPM Kemenkes (50%)', fill: '#f59e0b', fontSize: 10, position: 'insideTopRight' }} />
                <ReferenceLine yAxisId="right" y={80} stroke="#10b981" strokeDasharray="4 4" label={{ value: 'Target Layanan Prima (80%)', fill: '#10b981', fontSize: 10, position: 'insideTopRight' }} />
      ...
      {(activeTab === 'ALL' || activeTab === 'COHORT') && (
        <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-4">
          <div className="border-b border-slate-800 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <span className="text-xs px-3 py-1 rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/30 font-bold self-start sm:self-center">
              ...
              <div className="space-y-2">
                {LEVEL3_COHORT_DATA.map((item, idx) => (
                  ...
                  >
                    ...
                    <span className="text-white font-bold shrink-0 ml-2">{item.value} jiwa</span>
                  </div>
                ))}
              </div>
