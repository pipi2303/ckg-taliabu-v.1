...
export const ScenarioLabPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      {/* Header Banner */}
      <div className="bg-[#00201C] text-white p-6 rounded-xl border border-teal-900 shadow-2xs relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            ...
            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl flex items-center gap-2 transition cursor-pointer shadow-2xs shrink-0"
          >
      ...
      {toastMessage && (
        <div className="p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-xl text-amber-900 dark:text-amber-300 text-xs font-bold flex items-center gap-2 shadow-2xs">
      ...
      <div className="p-4 bg-white dark:bg-slate-900 border border-[#D8E5E2] dark:border-slate-800 rounded-xl text-xs space-y-1.5 shadow-2xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <div className="text-xs font-bold text-[#60716D] dark:text-slate-400 uppercase tracking-wider">
            ...
            <div
              ...
              className={`p-4 rounded-xl border text-xs space-y-2 cursor-pointer transition shadow-2xs ${
                selectedScenarioId === scen.id
                  ? 'bg-[#EBF7F2] dark:bg-teal-950/40 border-[#2E7D5B] dark:border-teal-700 text-[#102521] dark:text-white font-medium'
                  : 'bg-white dark:bg-slate-900 border-[#D8E5E2] dark:border-slate-800 text-[#102521] dark:text-slate-300 hover:border-[#BBE5D4]'
              }`}
            >
              ...
              <div className="text-[11px] text-[#60716D] dark:text-slate-400 flex items-center gap-1.5 font-medium">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-[#D8E5E2] dark:border-slate-800 rounded-xl p-6 space-y-6 shadow-2xs">
          <div className="border-b border-[#E8EFEB] dark:border-slate-800 pb-4">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-mono font-bold text-amber-800 dark:text-amber-400">ID: {activeScenario.id}</span>
              <span className="text-[10px] px-2 py-0.5 bg-[#F8FBFA] dark:bg-slate-800 text-[#60716D] dark:text-slate-400 rounded font-bold border border-[#D8E5E2] dark:border-slate-700">
                Mode: {activeScenario.mode}
              </span>
            </div>
            <h2 className="text-lg font-bold text-[#102521] dark:text-white mt-1">{activeScenario.name}</h2>
            <p className="text-xs text-[#60716D] dark:text-slate-400 mt-2 leading-relaxed">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-[#F8FBFA] dark:bg-slate-800 border border-[#D8E5E2] dark:border-slate-700 space-y-1 shadow-2xs">
              <div className="text-[11px] text-[#60716D] dark:text-slate-400 uppercase font-bold">
                Kondisi Baseline Saat Ini:
              </div>
              <div className="text-3xl font-extrabold text-[#102521] dark:text-white">
                {activeScenario.expectedRange.baselineRate}%
              </div>
              <div className="text-[11px] text-[#60716D] dark:text-slate-400 font-medium">
                {activeScenario.expectedRange.metricLabel}
              </div>
            </div>

            <div className="p-4 rounded-xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 space-y-1 shadow-2xs">
              <div className="text-[11px] text-amber-900 dark:text-amber-300 uppercase font-bold">
                Rentang Proyeksi Intervensi:
              </div>
              <div className="text-3xl font-extrabold text-amber-950 dark:text-amber-200">
              ...
              <div className="text-[11px] text-amber-900 dark:text-amber-300 font-medium">
                Arah: <strong className="text-amber-950 dark:text-amber-200 font-bold">{activeScenario.estimatedDirection}</strong> (Ketidakpastian: {activeScenario.uncertaintyRating})
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 text-xs">
            <h3 className="font-bold text-[#102521] dark:text-white uppercase tracking-wider flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-[#2E7D5B] dark:text-teal-400" />
              Asumsi Eksplisit Model:
            </h3>
            <div className="space-y-1.5">
              {activeScenario.assumptions.map((assump, idx) => (
                <div key={idx} className="p-2.5 bg-[#F8FBFA] dark:bg-slate-800 rounded-xl text-[#60716D] dark:text-slate-300 border border-[#D8E5E2] dark:border-slate-700 shadow-2xs font-medium">
                  • {assump}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="space-y-1.5">
              {activeScenario.dataLimitations.map((limit, idx) => (
                <div key={idx} className="p-2.5 bg-[#F8FBFA] dark:bg-slate-800 rounded-xl text-[#60716D] dark:text-slate-400 border border-[#D8E5E2] dark:border-slate-700 shadow-2xs font-medium">
                  • {limit}
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-[#D8E5E2] dark:border-slate-800 rounded-xl max-w-lg w-full p-6 space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-[#E8EFEB] dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2 text-sm font-bold text-[#102521] dark:text-white">
                <Play className="w-4 h-4 text-amber-600" />
                Jalankan Simulasi Skenario Baru
              </div>
              <button
                onClick={() => setShowNewSimulationModal(false)}
                className="text-[#60716D] hover:text-[#102521] dark:text-slate-400 dark:hover:text-white text-xs font-bold cursor-pointer"
              >
                Tutup
              </button>
            </div>
            ...
            <form onSubmit={handleRunCustomSimulation} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="text-[#102521] dark:text-white font-bold">Nama / Judul Skenario:</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-2.5 bg-[#F8FBFA] dark:bg-slate-800 border border-[#D8E5E2] dark:border-slate-700 rounded-xl text-[#102521] dark:text-white text-xs font-medium focus:outline-none focus:border-[#00201C]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[#102521] dark:text-white font-bold">Wilayah Kecamatan:</label>
                  <select
                    value={customRegion}
                    onChange={(e) => setCustomRegion(e.target.value)}
                    className="w-full p-2.5 bg-[#F8FBFA] dark:bg-slate-800 border border-[#D8E5E2] dark:border-slate-700 rounded-xl text-[#102521] dark:text-white text-xs font-semibold focus:outline-none focus:border-[#00201C] cursor-pointer"
                  >
                ...
                <div className="space-y-1">
                  <label className="text-[#102521] dark:text-white font-bold">Tipe Intervensi:</label>
                  <select
              ...
              <div className="space-y-1">
                <label className="text-[#102521] dark:text-white font-bold">Deskripsi Hipotetis:</label>
                <textarea
                  rows={3}
                  value={customDesc}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowNewSimulationModal(false)}
                  className="px-4 py-2 bg-[#F8FBFA] hover:bg-[#E8EFEB] dark:bg-slate-800 dark:hover:bg-slate-700 text-[#102521] dark:text-white rounded-xl cursor-pointer font-bold border border-[#D8E5E2] dark:border-slate-700"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl cursor-pointer font-bold shadow-2xs"
                >
                  Simulasikan
                </button>
              </div>
